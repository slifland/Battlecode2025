...
public class MopperMicro {
    ...
    //4. Attack square with ally robot - 0b10
    //5. Attack square without moving -0b1
    public static void integratedMopperMicro(RobotController rc) throws GameActionException {
        ...
        if(bestScore == -1) {
            ...
            if(rc.canAttack(target)) {
                ...
                targetedMopperMicro(rc, rc.getLocation().directionTo(target));
                if(rc.canAttack(target)) {
                    rc.attack(target);
                }
                else {
    ...
    public static int determineScore(RobotController rc, int x, int y) throws GameActionException {
        ...
        if(rc.onTheMap(temp)) {
            ...
            if(temp.isWithinDistanceSquared(nearbyRuin, 8)) {
                score |= 0b10000;
            }
